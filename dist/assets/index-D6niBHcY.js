...
`;function mt(e,t,n){let r=new me;r.moveTo(e[0].x,e[0].y);for(let t=1;t<e.length;t++)r.lineTo(e[t].x,e[t].y);r.closePath();let i=new u(r,1);i.rotateX(-Math.PI/2),i.computeVertexNormals();let a={uTime:{value:0},uWaves:{value:Array.from({length:dt},()=>new ve(0,0,0,0))},uWaveTint:{value:Array.from({length:dt},()=>new k(0,0,0))},uTerritory:{value:n.map(e=>new k(e.color).convertSRGBToLinear())},uTerrState:{value:new ve(1,1,1,1)},uRadius:{value:P.half},uDetail:{value:t.floorDetail},uCharge:{value:0},uHole:{value:new O(0,0,0)}},o=new g({color:1778738,metalness:.12,roughness:.72,emissive:0,envMapIntensity:.38});o.onBeforeCompile=e=>{Object.assign(e.uniforms,a),e.vertexShader=e.vertexShader.replace(`#include <common>`,`#include <common>
...
float hNoise(vec2 p){
  ...
  return mix(mix(hHash(i), hHash(i + vec2(1,0)), f.x),
        ...
        }`}),this.disc=new p(e,this.discMat),this.disc.position.y=.02,this.disc.renderOrder=7,this.haloMat=new d({transparent:!0,depthWrite:!1,blending:2,uniforms:{uTime:this._time,uAmount:this._amount},vertexShader:`
        varying vec2 vP;
        void main() {
          vP = position.xy;
          // Build the quad in view space so it always squarely faces us.
          vec4 c = modelViewMatrix * vec4(0.0, 0.0, 0.0, 1.0);
          c.xy += position.xy * ${(H.coreR*3.4).toFixed(3)};
          gl_Position = projectionMatrix * c;
        }`,fragmentShader:`
        precision mediump float;
        varying vec2 vP;
        ...
        }`}),this.halo=new p(new f(2,2),this.haloMat),this.halo.frustumCulled=!1,this.halo.renderOrder=9;let t=new x(H.radius*.965,H.radius,96,1);t.rotateX(-Math.PI/2),this.edgeMat=new d({transparent:!0,depthWrite:!1,side:2,blending:2,uniforms:{uTime:this._time,uAmount:this._amount},vertexShader:`varying vec2 vU; void main(){ vU = uv;
        gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,fragmentShader:`
        precision mediump float;
        varying vec2 vU;
        uniform float uTime; uniform float uAmount;
