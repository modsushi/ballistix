...
void main() {
  ...
  gl_FragColor = vec4(vColor * a * 2.6, a);
        ...
        void main() {
          ...
          }`)},r.customProgramCacheKey=()=>`sling-body`,this.slingMat=r,this.sling=new a(this.slingGeo,r,t),this._attr(this.slingGeo,`aState`,this.slingState,2),this.sling.castShadow=!1,this.sling.receiveShadow=!0,this.sling.frustumCulled=!1,this.sling.name=`slings`,this._placeInstances(),this.scene.add(this.body,this.crown,this.sling)}_attr(e,t,n,r){let i=new b(n,r);return i.setUsage(C),e.setAttribute(t,i),this._attrs.push(i),i}_placeInstances(){let e=new oe,t=new n,r=new O,i=new O(1,1,1),a=new O(0,1,0);this.bumpers.forEach((n,o)=>{r.set(n.x,P.floorY,n.z),t.setFromAxisAngle(a,n.seed*Math.PI),e.compose(r,t,i),this.body.setMatrixAt(o,e),this.crown.setMatrixAt(o,e),this.bumperState[o*2+1]=n.seed}),this.slings.forEach((n,o)=>{r.set(n.x,P.floorY,n.z),t.setFromAxisAngle(a,Math.atan2(-n.nx,-n.nz)),e.compose(r,t,i),this.sling.setMatrixAt(o,e),this.slingState[o*2+1]=n.seed}),this.body.instanceMatrix.needsUpdate=!0,this.crown.instanceMatrix.needsUpdate=!0,this.sling.instanceMatrix.needsUpdate=!0}reset(){for(let e of this.bumpers)e.flash=0,e.cool=0;for(let e of this.slings)e.flash=0,e.cool=0;this.phase=Qt,this.timer=V.downTime,this.lift=-2,this._warned=!1,this.justDeployed=!1,this.justRetracted=!1,this.justWarned=!1,this._applyLift()}get live(){return this.phase===en}get cycle01(){return this.phase===en?U(this.timer/V.upTime,0,1):this.phase===Qt?U(1-this.timer/V.downTime,0,1):+(this.phase===$t)}_applyLift(){let e=this.lift,t=e<=-1.999;for(let n of[this.body,this.crown,this.sling])n.position.y=e,n.visible=!t}_cycle(e){switch(this.justDeployed=!1,this.justRetracted=!1,this.justWarned=!1,this.timer-=e,this.phase){case Qt:this.timer<=V.warnTime&&!this._warned&&(this._warned=!0,this.justWarned=!0),this.timer<=0&&(this.phase=$t,this.timer=V.riseTime,this._warned=!1,this.justDeployed=!0);break;case $t:this.timer<=0&&(this.phase=en,this.timer=V.upTime);break;case en:this.timer<=0&&(this.phase=tn,this.timer=V.riseTime,this.justRetracted=!0);break;case tn:this.timer<=0&&(this.phase=Qt,this.timer=V.downTime)}let t=1;this.phase===Qt?t=0:this.phase===$t?t=1-U(this.timer/V.riseTime,0,1):this.phase===tn&&(t=U(this.timer/V.riseTime,0,1));let n=this.phase===$t?1-(1-t)**3:t;this.lift=-2*(1-n),this._applyLift()}collide(e,t){if(this.phase!==en)return!1;let n=z.radius;for(let r of this.bumpers){if(r.cool>0)continue;let i=e.x-r.x,a=e.z-r.z,o=r.r+n,s=i*i+a*a;if(s>o*o)continue;let c=Math.sqrt(s),l,u;if(c>1e-5)l=i/c,u=a/c;else{let t=Math.hypot(e.vx,e.vz)||1;l=-e.vx/t,u=-e.vz/t}let d=Math.hypot(e.vx,e.vz)||1,f=(e.vx*l+e.vz*u)/d,p=e.vx/d-2*f*l,m=e.vz/d-2*f*u,h=p*.42+l*.72,g=m*.42+u*.72,_=Math.atan2(g,h)+K(-V.scatter,V.scatter),v=U(d+V.kick,V.kickFloor,z.maxSpeed);return e.vx=Math.cos(_)*v,e.vz=Math.sin(_)*v,e.speed=v,e.x=r.x+l*(o+.02),e.z=r.z+u*(o+.02),e.registerImpact(-l,-u,1),r.flash=1,r.cool=V.cool,t.push({type:`bumper`,x:e.x,z:e.z,nx:l,nz:u,speed:v,el:r}),!0}for(let r of this.slings){let i=e.x-r.x,a=e.z-r.z,o=i*r.nx+a*r.nz,s=i*r.tx+a*r.tz;if(o<-r.depth-n||o>n||Math.abs(s)>r.half+n)continue;let c=U(o,-r.depth,0),l=U(s,-r.half,r.half),u=o-c,d=s-l,f=u*u+d*d,p,m,h;if(f>1e-8){if(f>n*n)continue;let e=Math.sqrt(f);p=u/e,m=d/e,h=n-e+.02}else{let e=-o,t=o+r.depth,i=r.half-Math.abs(s);e<=t&&e<=i?(p=1,m=0,h=e+n+.02):t<=i?(p=-1,m=0,h=t+n+.02):(p=0,m=Math.sign(s)||1,h=i+n+.02)}let g=r.nx*p+r.tx*m,_=r.nz*p+r.tz*m,v=e.vx*g+e.vz*_<0;if(e.x+=g*h,e.z+=_*h,p>.5&&v&&r.cool<=0){let n=U(s/r.half,-1,1),i=r.nx+r.tx*n*V.slingSpread,a=r.nz+r.tz*n*V.slingSpread,o=Math.hypot(i,a)||1,c=V.slingSpeed;return e.vx=i/o*c,e.vz=a/o*c,e.speed=c,e.registerImpact(-r.nx,-r.nz,1.3),r.flash=1,r.cool=V.cool,t.push({type:`sling`,x:e.x,z:e.z,nx:r.nx,nz:r.nz,speed:c,u01:n*.5+.5,el:r}),!0}if(v){let n=e.vx*g+e.vz*_;return e.vx-=2*n*g,e.vz-=2*n*_,e.registerImpact(-g,-_,.5),t.push({type:`wall`,orb:e,x:e.x,z:e.z,nx:g,nz:_,speed:e.speed,goal:-1}),!0}return!1}return!1}update(e,t){this._time&&(this._time.value=t),this._cycle(e);for(let t=0;t<this.bumpers.length;t++){let n=this.bumpers[t];n.cool=Math.max(0,n.cool-e),n.flash=Math.max(0,n.flash-e*3.6),this.bumperState[t*2]=n.flash}for(let t=0;t<this.slings.length;t++){let n=this.slings[t];n.cool=Math.max(0,n.cool-e),n.flash=Math.max(0,n.flash-e*3),this.slingState[t*2]=n.flash}for(let e of this._attrs)e.needsUpdate=!0}dispose(){this.bodyGeo.dispose(),this.crownGeo.dispose(),this.slingGeo.dispose(),this.bodyMat.dispose(),this.crownMat.dispose(),this.slingMat.dispose(),this.scene.remove(this.body,this.crown,this.sling)}},rn=V.warnTime,Y=11824127,an=class{constructor(e,t=10){this.items=[];let n=new x(.78,1,48,1);n.rotateX(-Math.PI/2),this.geo=n;for(let r=0;r<t;r++){let t=new d({transparent:!0,depthWrite:!1,blending:2,side:2,uniforms:{uColor:{value:new k(1,1,1)},uFade:{value:0}},vertexShader:`varying vec2 vU; void main(){ vU=uv;
...
`,hn=class{constructor(e,t){this.scene=e,this.preset=t,this.count=B.perQuadrant*sn,this.bricks=[],this.liveCount=0,this.spawned=0,this._nextAt=0,this.justSpawned=[];let{w:r,d:i,h:a}=B;this.geo=Yt(r,i,a,B.bevel),this.auraGeo=Yt(r+.16,i+.16,a+.1,B.bevel),this.state=new Float32Array(this.count*3),this.tint=new Float32Array(this.count*3),this._attrs=[],this.shell=this._buildShell(),this.aura=this._buildAura(),e.add(this.shell,this.aura),this._m=new oe,this._q=new n,this._pos=new O,this._scl=new O,this._col=new k}_attr(e,t,n,r){let i=new b(n,r);return i.setUsage(C),e.setAttribute(t,i),this._attrs.push(i),i}_buildShell(){this._attr(this.geo,`aState`,this.state,3),this._attr(this.geo,`aTint`,this.tint,3);let e=new g({color:1252135,metalness:.9,roughness:.34,envMapIntensity:.85}),t=new O(B.w,B.h,B.d);e.onBeforeCompile=e=>{e.uniforms.uDim={value:t},e.vertexShader=e.vertexShader.replace(`#include <common>`,`#include <common>
          ...
          ${pn}`).replace(`#include <roughnessmap_fragment>`,`#include <roughnessmap_fragment>
          {
...
float hNoise(vec2 p){
  ...
  return mix(mix(hHash(i), hHash(i + vec2(1,0)), f.x),
...
}`,Cn=class{constructor(e,t){this.scene=e,this.preset=t,this.x=0,this.z=0,this.phase=_n,this.timer=H.first,this.strength=0,this.quadrant=Math.floor(Math.random()*4),this.justWarned=!1,this.justOpened=!1,this.justClosed=!1,this._warned=!1,this.root=new j,this.root.visible=!1,e.add(this.root),this._time={value:0},this._amount={value:0},this._build()}_build(){this.coreMat=new d({uniforms:{uTime:this._time,uAmount:this._amount},vertexShader:`
        ...
        }`}),this.edge=new p(t,this.edgeMat),this.edge.position.y=P.floorY-P.playY+.045,this.edge.renderOrder=4,this.root.add(this.core,this.disc,this.edge)}reset(){this.phase=_n,this.timer=H.first,this.strength=0,this.justWarned=this.justOpened=this.justClosed=!1,this._warned=!1,this.root.visible=!1}get live(){return this.strength>.001}get cycle01(){return this.phase===_n?U(1-this.timer/H.cooldown,0,1):this.phase===bn?U(this.timer/H.duration,0,1):this.phase===vn?1:this.strength}_place(e){this.quadrant=(this.quadrant+1)%4;let t=0,n=0;for(let r=0;r<12;r++){let r=(this.quadrant+K(.18,.82))*Math.PI*.5;if(t=Math.cos(r)*H.spawnR,n=Math.sin(r)*H.spawnR,!e||this._clearOf(e,t,n))break}this.x=t,this.z=n,this.root.position.set(t,P.playY,n)}_clearOf(e,t,n){for(let r of e.standing()){let e=U(t-r.x,-r.hw,r.hw),i=U(n-r.z,-r.hd,r.hd);if(Math.hypot(t-r.x-e,n-r.z-i)<H.coreR+H.clearance)return!1}return!0}affect(e,t){if(this.strength<=.001)return!1;let n=this.x-e.x,r=this.z-e.z,i=n*n+r*r,a=H.radius;if(i>a*a||i<1e-6)return!1;let o=Math.sqrt(i),s=o/a,c=(1-s*s)*wn(o),l=H.pull*c*this.strength*t,u=e.speed||Math.hypot(e.vx,e.vz)||1;e.vx+=n/o*l,e.vz+=r/o*l;let d=Math.hypot(e.vx,e.vz)||1;return e.vx=e.vx/d*u,e.vz=e.vz/d*u,!0}update(e,t,n){switch(this.justWarned=this.justOpened=this.justClosed=!1,this._time.value=t,this.timer-=e,this.phase){case _n:this.timer<=H.warnTime&&!this._warned&&(this._warned=!0,this._place(n),this.justWarned=!0),this.timer<=0&&(this.phase=yn,this.timer=H.openTime,this._warned=!1,this.justOpened=!0,this.root.visible=!0);break;case yn:this.strength=1-U(this.timer/H.openTime,0,1),this.timer<=0&&(this.phase=bn,this.timer=H.duration,this.strength=1);break;case bn:this.timer<=0&&(this.phase=xn,this.timer=H.openTime,this.justClosed=!0);break;case xn:this.strength=U(this.timer/H.openTime,0,1),this.timer<=0&&(this.phase=_n,this.timer=H.cooldown,this.strength=0,this.root.visible=!1)}if(!this.root.visible)return;this._amount.value=this.strength;let r=this.strength**.65;this.core.scale.setScalar(r),this.disc.scale.set(r,1,r),this.disc.rotation.y=t*.35,this.edge.scale.set(.35+r*.65,1,.35+r*.65)}dispose(){this.core.geometry.dispose(),this.disc.geometry.dispose(),this.edge.geometry.dispose(),this.coreMat.dispose(),this.discMat.dispose(),this.edgeMat.dispose(),this.scene.remove(this.root)}};function wn(e){let t=U(e/(H.coreR*2.2),0,1);return t*t*(3-2*t)}var Tn=`
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
